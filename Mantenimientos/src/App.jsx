...
// ─── COMPONENT ────────────────────────────────────────────────────────────
const DARK  = { bg:"#09090e", card:"#0f0f17", line:"#1c1c2a", text:"#e0d8cc", sub:"#555", gold:"#C8A96E" };
const LIGHT = { bg:"#f4f4f0", card:"#ffffff",  line:"#e0ddd8", text:"#1a1a1a", sub:"#888", gold:"#9a6f2e" };
...
_Sistema de Gestión de Taller — Mercedes-Benz_`;
  ...
  const inp = { background:card, border:`1px solid ${line}`, color:T.text, borderRadius:6, padding:"7px 10px", fontSize:12, fontFamily:"monospace", outline:"none" };

  return (
    <div style={{ background:bg, minHeight:"100vh", fontFamily:"monospace", color:T.text }}>
      {/* Overlay para cerrar el buscador */}
      {modelOpen && <div onClick={()=>setModelOpen(false)} style={{ position:"fixed", inset:0, zIndex:40 }} />}

      {/* HEADER */}
      <div style={{ background: theme==="light"?"#ffffff":"#0d0d16", borderBottom:`1px solid ${line}`, padding:"12px 16px", display:"flex", alignItems:"center", gap:12, position:"sticky", top:0, zIndex:9 }}>
        <img src={LOGO_SRC} alt="Ramos y Ramos" style={{ width:36, height:36, borderRadius:"50%", flexShrink:0, objectFit:"cover" }} />
        <div>
          <div style={{ fontWeight:"bold", letterSpacing:2, fontSize:13, color:T.text }}>RAMOS Y RAMOS</div>
          <div style={{ fontSize:9, color:T.sub, letterSpacing:3 }}>TALLER ESPECIALIZADO · MERCEDES-BENZ</div>
        </div>
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
          {doneN > 0 && (
            <div style={{ fontSize:10, padding:"3px 11px", borderRadius:20, border:`1px solid ${isComplete?"#4ade80":G}`, color:isComplete?"#4ade80":G, background:isComplete?(theme==="light"?"#dcfce7":"#14532d"):(theme==="light"?"#fef9ee":"#1a1a2a") }}>
              {isComplete ? "✓ COMPLETO" : pct+"%"}
            </div>
          )}
          <button onClick={toggleTheme} title="Cambiar tema"
            style={{ padding:"5px 8px", borderRadius:8, border:`1px solid ${line}`, background:card, color:T.sub, fontSize:14, cursor:"pointer", lineHeight:1 }}>
            {theme==="dark" ? "☀️" : "🌙"}
          </button>
        </div>
      </div>
      ...
      {/* DATOS VEHÍCULO */}
      <div style={{ padding:"12px 16px", borderBottom:`1px solid ${line}`, background: theme==="light"?"#f8f7f4":"#0c0c14" }}>
