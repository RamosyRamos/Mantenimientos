...
function LoginScreen({ onLogin }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const queryNombre = nombre.trim();
      console.log("[Auth] Querying nombre:", queryNombre);
      const res = await fetch(
        `${SURL}/rest/v1/usuarios?nombre=ilike.${encodeURIComponent(queryNombre)}&activo=eq.true&select=*`,
      ...
      console.log("[Auth] User match for app:", user ? `${user.nombre} (app=${user.app})` : "none");
      if (!user) { setError("Nombre o contraseña incorrectos"); setLoading(false); return; }
      const parts = (user.password || "").split(":");
      const salt = parts[0];
      const storedHash = parts.slice(1).join(":");
      console.log("[Auth] salt:", salt, "storedHash:", storedHash);
      if (!salt || !storedHash) { setError("Nombre o contraseña incorrectos"); setLoading(false); return; }
      const computed = await sha256hex(salt + password);
      console.log("[Auth] computed:", computed, "match:", computed === storedHash);
      if (computed !== storedHash) { setError("Nombre o contraseña incorrectos"); setLoading(false); return; }
