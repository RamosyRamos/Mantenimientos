...
function PinScreen({ onUnlock, onRecent }) {
  const [pin, setPin]     = useState("");
  const [error, setError] = useState(false);
  const [shake, setShake] = useState(false);
  const [mode, setMode]   = useState("new"); // "new" | "recent"
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#09090e", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"monospace", padding:24 }}>
      ...
      <div style={{ fontSize:9, color:"#555", letterSpacing:3, marginBottom:8 }}>TALLER ESPECIALIZADO · MERCEDES-BENZ</div>

      {/* Selector de modo */}
      <div style={{ display:"flex", gap:8, marginBottom:32 }}>
        <button onClick={() => { setMode("new"); setPin(""); setError(false); }}
          style={{ padding:"6px 16px", borderRadius:20, border:`1px solid ${mode==="new"?"#C8A96E":"#2a2a3a"}`, background:mode==="new"?"#C8A96E18":"transparent", color:mode==="new"?"#C8A96E":"#555", fontSize:11, letterSpacing:1, cursor:"pointer", fontFamily:"monospace" }}>
          + Nuevo servicio
        </button>
        <button onClick={() => { setMode("recent"); setPin(""); setError(false); }}
          style={{ padding:"6px 16px", borderRadius:20, border:`1px solid ${mode==="recent"?"#C8A96E":"#2a2a3a"}`, background:mode==="recent"?"#C8A96E18":"transparent", color:mode==="recent"?"#C8A96E":"#555", fontSize:11, letterSpacing:1, cursor:"pointer", fontFamily:"monospace" }}>
          🕐 Ver historial
        </button>
      </div>
...
export default function App() {
      ...
      <div style={{ position:"fixed", inset:0, zIndex:200, background:"#000a" }} onClick={() => setShowRecent(false)}>
        <div onClick={e => e.stopPropagation()} style={{ position:"absolute", top:0, right:0, width:"min(380px,100vw)", height:"100vh", background:"#0f0f17", borderLeft:"1px solid #1c1c2a", display:"flex", flexDirection:"column" }}>
          <div style={{ padding:"14px 16px", borderBottom:"1px solid #1c1c2a", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <button onClick={() => setShowRecent(false)} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #1c1c2a", background:"transparent", color:"#555", fontSize:14, cursor:"pointer" }}>✕</button>
          </div>
  ...
  return <MainApp recentPanel={recentPanel} showRecent={showRecent} setShowRecent={setShowRecent} fetchRecent={fetchRecent} recentList={recentList} recentLoading={recentLoading} />;
}

function MainApp({ recentPanel, showRecent, setShowRecent, fetchRecent, recentList, recentLoading }) {
