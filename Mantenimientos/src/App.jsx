...
// ── Login (F4 Sesión E: teléfono + OTP por SMS con Supabase Auth) ─────────────
// Patrón copiado de Taller (App.jsx LoginScreen / resolverSesionDesdeAuth).
// La `session` de la app NO cambia de forma ni de clave ('ryr_session' =
// {id, username, nombre, rol, app}); solo cambia CÓMO se obtiene:
// signInWithOtp → verifyOtp → rpc mi_usuario() (solo authenticated).
//
// REGLA DE APP (distinta a Taller): Mantenimientos acepta CUALQUIER `app`. Los
// mecánicos son app 'taller' y llegan acá por el link de la orden; la columna
// `app` deja de gatear el acceso y queda como dato legacy. Solo se rechaza
// mi_usuario() = null (cuenta Auth sin usuarios.auth_id, o inactivo).
async function resolverSesionDesdeAuth() {
  const { data, error } = await supabase.rpc('mi_usuario');
  if (error) return { error };
  if (!data || !data.id) {
    try { await supabase.auth.signOut({ scope: 'local' }); } catch { /* sin red: igual no hay sesión de app */ }
    return { rechazo: 'sin_enlace', mensaje: MENSAJE_RECHAZO.sin_enlace };
  }
  const session = { id: data.id, username: data.username ?? null, nombre: data.nombre, rol: data.rol, app: data.app };
...
function LoginScreen({ onLogin, mensajeInicial = '' }) {
  ...
  const [aviso,    setAviso]    = useState('');     // "Te lo mandamos por SMS" (solo si hubo fallback de canal)
  const [canalUsado, setCanalUsado] = useState(CANAL_OTP);
      ...
      {paso === 1 ? (
        ...
        <form onSubmit={verificar} style={{ width:"100%", maxWidth:300, display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <input type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]*" value={codigo}
            onChange={e => { setCodigo(soloDigitos(e.target.value).slice(0, 6)); setError(''); }}
            placeholder="······" maxLength={6} autoFocus
            style={{ ...fieldStyle, textAlign:"center", letterSpacing:10, fontSize:22 }} />
          {error && <div style={{ fontSize:11, color:"#ef4444", textAlign:"center", letterSpacing:0.5, lineHeight:1.5 }}>{error}</div>}
          <button type="submit" disabled={!puedeEntrar} style={btnStyle(puedeEntrar)}>
...
// Pantalla mínima mientras getSession() + mi_usuario() resuelven al arrancar:
// evita el parpadeo del login en un teléfono ya logueado.
function ResolviendoSesion() {
  return (
    <div style={{ minHeight:"100vh", background:"#0B0B0D", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"monospace" }}>
      <img src={LOGO_SRC} alt="" style={{ width:80, height:80, borderRadius:"50%", marginBottom:14, objectFit:"cover", opacity:0.85 }} />
      <div style={{ fontSize:9, color:"#555", letterSpacing:3 }}>CARGANDO…</div>
    </div>
  );
}
...
export default function App() {
  // F4 Sesión E: la sesión de la app ('ryr_session' = {id, username, nombre, rol,
  // app}, misma forma que siempre) se DERIVA de la sesión de Supabase Auth.
  // Ya NO existe la sesión sintetizada desde el link de Taller
  // (?mecanico=&orden_id=): `mecanico` de la URL es solo el técnico asignado
  // del servicio (mechName → servicios.mecanico), nunca identidad.
  const [session, setSession] = useState(() => {
    try { const s = localStorage.getItem(SESSION_KEY); return s ? JSON.parse(s) : null; } catch { return null; }
  });
  const [authListo, setAuthListo] = useState(false);   // ya se consultó getSession() (+ mi_usuario)
  const [loginMsg,  setLoginMsg]  = useState('');      // motivo de rechazo para el login
  // Los query params del link de Taller (?placa=&orden_id=&mecanico=…&servicio=)
  // se conservan durante el login: nadie los toca, pero por si alguna vez la URL
  // quedara limpia se guardan acá y se reponen al entrar.
  const searchInicialRef = useRef(window.location.search);

  const limpiarSesionLocal = useCallback(() => {
    localStorage.removeItem(SESSION_KEY);
    localStorage.removeItem('ryr_session_source');   // clave legacy de la sintetizada
    setSession(null);
  }, []);

  const logout = useCallback(async () => {
    try { await supabase.auth.signOut({ scope: 'local' }); } catch (e) { console.warn('[Auth] signOut:', e); }
    limpiarSesionLocal();
  }, [limpiarSesionLocal]);

  const alEntrar = useCallback((s) => {
    setLoginMsg('');
    if (!window.location.search && searchInicialRef.current) {
      try { window.history.replaceState(null, '', window.location.pathname + searchInicialRef.current); } catch { /* nada */ }
    }
    setSession(s);
  }, []);

  // Arranque: sin sesión Auth → login SIEMPRE (se borra cualquier ryr_session
  // vieja: único re-login de la migración). Con sesión Auth → mi_usuario(): fila
  // → session; null → signOut + mensaje; error de RED → fail-open (se conserva
  // la sesión local que había; la revalidación reintenta al volver a la pestaña).
  useEffect(() => {
    let vivo = true;
    (async () => {
      try {
        const { data: { session: sAuth } } = await supabase.auth.getSession();
        if (!vivo) return;
        if (!sAuth) { limpiarSesionLocal(); return; }
        const r = await resolverSesionDesdeAuth();
        if (!vivo) return;
        ...
        if (vivo) setAuthListo(true);
      }
    })();
    return () => { vivo = false; };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // SIGNED_OUT (logout propio, sesión revocada, refresh imposible) → login.
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((event) => {
      if (event === 'SIGNED_OUT') limpiarSesionLocal();
    });
    return () => sub?.subscription?.unsubscribe();
  }, [limpiarSesionLocal]);

  // REVALIDACIÓN DE SESIÓN — Parte A del fix de seguridad (migración
  // 20260818000001 de Taller). La baja de un usuario es desactivación y NUNCA
  // DELETE: sin esto, un desactivado seguía con la app usable hasta cerrar
  // sesión a mano. F4: `mi_usuario()` (solo authenticated) devuelve la fila
  // propia por auth.uid() con `activo = true` ⇒ NULL = desactivado (o cuenta
  // Auth des-enlazada). Ya no hay guard por "source taller": la sintetizada
  // no existe más y todo el mundo se revalida.
  // ⚠ FAIL-OPEN: solo null SIN error expulsa. Red caída, JWT sin refresh o env
  // sin configurar NO echan a nadie.
  useEffect(() => {
  ...
  if (!authListo) return <ResolviendoSesion />;
  if (!session) return <LoginScreen onLogin={alEntrar} mensajeInicial={loginMsg} />;
...
function MainApp({ session, onLogout }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch(`${SURL}/rest/v1/mant_recetas?select=*&order=serie,orden`).then(r => r.json()),
      apiFetch(`${SURL}/rest/v1/mant_items?select=*&order=orden`).then(r => r.json()),
  ...
  // cameFromTaller es solo un MODO DE UI (llegó con orden_id/mecanico): esconde
  // los botones de admin para que el mecánico vea la pantalla limpia. La
  // identidad ya no depende de la URL (F4): viene de mi_usuario() vía Auth.
  const showAdminButtons = !cameFromTaller && (session?.rol === 'admin' || session?.rol === 'jefe');
  // Modo edición de aprobados y bandeja 🗂 Borradores: rol 'jefe' REAL, el de
  // la fila de usuarios (mi_usuario), nunca por nombre. Antes existía `esTavo`
  // (nombre === 'Gustavo Ramos'), que con la sesión sintetizada se encendía
  // pegando ?mecanico=Gustavo%20Ramos en la URL — eliminado.
  const esJefeReal = session?.rol === 'jefe';
  ...
  useEffect(() => {
    ...
    // Borradores "míos": servicios.mecanico = el nombre de quien está logueado
    // (para el mecánico coincide con el técnico asignado del link).
    const mechFilter = encodeURIComponent(session.nombre);
    apiFetch(`${API_URL}/rest/v1/servicios?estado=eq.borrador&mecanico=eq.${mechFilter}&order=created_at.desc`)
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    apiFetch(`${API_URL}/rest/v1/servicios?id=eq.${encodeURIComponent(svcId)}&select=*`)
      .then(r => r.json())
      .then(rows => {
  ...
  const recargarServicio = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/rest/v1/servicios?id=eq.${id}&select=*`);
  ...
  guardarAhoraRef.current = async (intento = 0) => {
    ...
    try {
      ...
      // F4: el sello es de QUIEN ESTÁ LOGUEADO (identidad), nunca del mechName
      // (que es el técnico asignado del servicio, un dato).
      const byGrpMap = sellarRevisiones(crudo, itemSnapRef.current, itemStampsRef.current, session?.nombre || null, new Date().toISOString());
  ...
  // send-push autoriza por x-session-id de un usuario activo (hasta F5, que
  // verificará el JWT). Hasta F4 esta llamada salía con Bearer anon y SIN
  // x-session-id → 401 "unauthorized" (reproducido el 13/9): ningún push de
  // Mantenimientos llegaba. apiFetch ahora manda JWT + x-session-id.
  const notifyPush = async (userNombres, title, body) => {
    try {
      await apiFetch(`${SUPABASE_URL}/functions/v1/send-push`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/rest/v1/servicios?estado=eq.pendiente&aprobado=eq.false&order=created_at.desc`);
      const data = await res.json();
      ...
      try {
        const resCorr = await apiFetch(`${API_URL}/rest/v1/correcciones_aceite?estado=eq.pendiente&order=created_at.desc`);
  ...
  const proponerCorreccionAceite = async () => {
    ...
    try {
      await apiFetch(`${API_URL}/rest/v1/correcciones_aceite`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          // F4: autoría = quien está logueado (con uuid real; antes id null desde el link)
          propuesto_por: session?.nombre || null,
          propuesto_por_id: session?.id ?? null,
        }),
      });
      setCorrEnviada(true); setCorrOpen(false);
    } catch(e) {
      if (e?.status === 409) { setCorrError("Ya hay una corrección pendiente para este modelo."); setCorrSending(false); return; }
      console.error("[proponerCorreccionAceite]", e); setCorrError(`No se pudo enviar (${mensajeError(e)}). Intentá de nuevo.`);
    }
  ...
  const resolverCorreccionAceite = async (correccionId, aprobador, aprobar) => {
    ...
    try {
      // supabase.rpc lleva el JWT solo (rol authenticated); el RPC es SECURITY DEFINER.
      const { error } = await supabase.rpc('aprobar_correccion_aceite', { p_correccion_id: correccionId, p_aprobador: aprobador, p_aprobar: aprobar });
      if (error) { console.error("[aprobar_correccion_aceite]", error); alert("No se pudo resolver la corrección. Intentá de nuevo."); }
    } catch(e) { console.error("[aprobar_correccion_aceite]", e); alert("No se pudo resolver la corrección. Intentá de nuevo."); }
  ...
  const fetchBorradores = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`${API_URL}/rest/v1/servicios?estado=eq.borrador&select=*&order=created_at.desc`);
      const data = await res.json();
  ...
  const fetchCompleted = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/rest/v1/servicios?estado=eq.aprobado&order=created_at.desc&limit=100`);
      const data = await res.json();
  ...
  const fetchVerTodos = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/rest/v1/servicios?select=*&order=created_at.desc&limit=500`);
      {
        const data = await res.json();
              ...
              {corrList.map(c => (
                <div key={c.id} style={{ marginBottom:10, padding:"10px 12px", borderRadius:8, border:"1px solid #C8A96E30", background:"#C8A96E08" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:5, opacity:corrResolvingId === c.id ? 0.5 : 1 }}>
                    {/* F4: aprueba QUIEN ESTÁ LOGUEADO (identidad de Auth), ya no un botón por nombre */}
                    <button onClick={() => resolverCorreccionAceite(c.id, session?.nombre || "—", true)} disabled={corrResolvingId === c.id}
...
_Progreso: ${doneN}/${total} ítems (${pct}%)_`;
  ...
  // El texto del informe NO lleva link: el Taller agrega el del Portal
  // (portal.ramosyramoscr.com/?m=<servicios.id>) al compartirlo con el cliente.
  // La vista pública vieja /servicio/<slug> (clientUrl) queda solo para la UI.
  const buildInformeMarkdown = ({ correccion = false } = {}) => {
    const issueTasks = tasks.filter(t => taskStatus[t.id] === "issue" || taskIssue[t.id]);
  ...
  const regenerarInformeOrden = async ({ correccion = false } = {}) => {
    ...
    } catch (e) { console.warn("[regenerarInformeOrden] PATCH ordenes falló:", e); }
  };
  ...
  const enviarAOrden = async () => {
    ...
    try {
      ...
      let checkData = null;
      try {
        const checkRes = await apiFetch(`${SUPABASE_URL}/rest/v1/ordenes?id=eq.${ordenId}&select=informe_mantenimiento,vehiculo_id,vehiculos(patente)`);
                ...
                <div style={{ marginTop:4 }}>
                  ...
                  {modoRevision && !aprobado && (
                    <div style={{ marginBottom:10, padding:"14px", borderRadius:8, border:"1px solid #4ade8040", background:"#4ade8006" }}>
                      ...
                      <div style={{ fontSize:9, color:"#555", marginBottom:10, textAlign:"center" }}>La aprobación queda a nombre de quien está logueado:</div>
