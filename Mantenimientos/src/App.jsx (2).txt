...
_Progreso: ${doneN}/${total} ítems (${pct}%)_`;
  ...
  const confirmSig = async () => {
    ...
    // Guardar automáticamente en Supabase al confirmar firma
    try {
      const svcData = buildServiceData();
      let slug;
      if (editingId && clientUrl) {
      ...
      const res = await fetch(
        editingId
          ? `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/servicios?id=eq.${editingId}`
          : `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/servicios`,
        {
          method: editingId ? "PATCH" : "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            modelo: svcData.vehiculo.modelo,
            motor: svcData.vehiculo.motor,
            mecanico: svcData.mecanico || mechName,
            fecha,
            servicio_codigo: svcData.servicio.codigo,
            servicio_desc: svcData.servicio.descripcion,
            km: svcData.vehiculo.km,
      ...
      const data = await res.json();
      const savedId = data?.[0]?.id || editingId;
      if (savedId || slug) {
        const url = `${import.meta.env.VITE_APP_URL || window.location.origin}/servicio/${slug}`;
        setClientUrl(url);
        if (!editingId && data?.[0]?.id) setEditingId(data[0].id);
      }
